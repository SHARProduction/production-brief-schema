...
Node.js20+; validator has no runtime dependencies. Dataset loading requires the test dependencies documented in its README. Browser builds reuse the same schema and synthetic examples. Hosted dataset/tool links will be added after their deployments are verified; a local build is not a hosted application.
